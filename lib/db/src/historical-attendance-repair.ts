...
function hasValidOvernightCheckout(
  ...
  return end < start && current.minutes < end;
}
...
export function repairHistoricalAttendanceOrder(
  ...
  const alreadyApplied = sqlite
    ...
    .get(MIGRATION_NAME);
  if (alreadyApplied) {
    return { skipped: true, repairedEvents: 0, repairedPairs: 0 };
  }
  ...
  const repairs = sqlite.transaction(() => {
    ...
    for (const dayEvents of eventsByEmployeeAndDate.values()) {
      ...
      if (hasValidOvernightCheckout(first, employeeSchedules)) continue;
    ...
    return { repairedEvents, repairedPairs };
  })();

  return { skipped: false, ...repairs };
}